...
static int chk_inifile(GKeyFile *cfg_ini, const char *file_ini, 
		GKeyFileFlags flags) {
    GError *error = NULL;
    
    if (!g_key_file_load_from_file(cfg_ini, file_ini, flags, &error)) {
...
static int chk_inistr(GKeyFile *cfg_ini, const char *buf_ini, 
		GKeyFileFlags flags) {
    GError *error = NULL;
...
static int yamltokens_to_list(yaml_parser_t *yamlparser, GData **datalist) {
	char *name_block = "default", *keyX = "default", *tk;
	char buf_tmp[256];
	int is_value = 0;
	...
	do {
		...
		switch (token_yaml.type) {
			...
			case YAML_KEY_TOKEN: is_value = 0; break;
			case YAML_VALUE_TOKEN: is_value = 1; break;
			// Block delimeters
			...
			case YAML_SCALAR_TOKEN: 
				...
				if (0 == is_value) {
					...
					snprintf(buf_tmp, sizeof(buf_tmp), "%s:%s", name_block, keyX);
					g_datalist_set_data(datalist, strdup(buf_tmp), strdup(tk));
				}
				break;
			// Others
			//default: 
		}
		if (YAML_STREAM_END_TOKEN != token_yaml.type)
			yaml_token_delete(&token_yaml);
	} while (YAML_STREAM_END_TOKEN != token_yaml.type);
	yaml_token_delete(&token_yaml);
	
	return 0;
}

static int yamlevents_to_list(yaml_parser_t *yamlparser, GData **datalist) {
	char *name_block = "default", *keyX = "default", *tk;
	char buf_tmp[256];
	int is_key = 1;
	yaml_event_t event_yaml;
	do {
		if (!yaml_parser_parse(yamlparser, &event_yaml)) {
			printf("Parser error %d\n", yamlparser->error);
			exit(EXIT_FAILURE);
		}

		switch (event_yaml.type) { 
			...
			case YAML_MAPPING_START_EVENT:
				name_block = strdup(keyX); is_key = 1; break;
			case YAML_MAPPING_END_EVENT: is_key = 1; break;
			// Data
			//case YAML_ALIAS_EVENT: break;
			case YAML_SCALAR_EVENT:
				tk = event_yaml.data.scalar.value;
				if (1 == is_key) {
					keyX = strdup(tk);
					is_key = 0;
				} else {
		...
		if (YAML_STREAM_END_EVENT != event_yaml.type)
			yaml_event_delete(&event_yaml);
	} while (YAML_STREAM_END_EVENT != event_yaml.type);
	yaml_event_delete(&event_yaml);
...
int main(int argc, char **argv) {
    ...
    if (NULL == (fIn = fopen(rc_buf, "r")))
		...
		if (chk_inifile(cfg_ini, rc_buf, G_KEY_FILE_KEEP_COMMENTS)) {
		//if (chk_inistr(cfg_ini, buf_ini, G_KEY_FILE_KEEP_COMMENTS)) {
    ...
    if (NULL == (fIn = fopen(rc_buf, "r")))
		...
		if (!yaml_parser_initialize(&yamlparser))
			...
			yaml_parser_set_input_string(&yamlparser, buf_yaml, strlen(buf_yaml));
			GData *datalist = NULL;
			g_datalist_init(&datalist);
			
			//yamlevents_to_list(&yamlparser, &datalist);
			yamltokens_to_list(&yamlparser, &datalist);
